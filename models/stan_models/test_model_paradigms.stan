...
parameters {
  ...
  real beta_v_true1;                    // Vowel true=1 emission parameter
  real beta_v_true0;                    // Vowel true=0 emission parameter
  real beta_c_true1;                    // Consonant true=1 emission parameter
  real beta_c_true0;                    // Consonant true=0 emission parameter
...
transformed parameters {
  ...
  for (i in 1:N_principal_parts) {
    for (j in 1:N_principal_parts) {
      if (i == j) {
        paradigm_influence[i, j] = 0.0;  // Fix diagonal to 0.0 (a principal part does not influence itself)
      } else {
...
model {
  ...
  // Priors for emission parameters (replacing spline priors)
  ...
  for (v in 1:N_verbs) {
    ...
    for (s in 1:N_states) {
      ...
      // SIMPLIFIED: Compute emission logits without splines
      real logit_v;
      if (v_true == 1) {
        logit_v = beta_v_true1;
      } else {
        logit_v = beta_v_true0;
      }
      logit_v += beta_dialects[dialect_id[idx]];

      real logit_c;
      if (c_true == 1) {
        logit_c = beta_c_true1;
      } else {
        logit_c = beta_c_true0;
      }
    ...
    for (t in 2:T) {
        ...
        if (other_pp != curr_pp) {
          ...
          if (other_latest_idx > 0) {
            ...
            for (s in 1:N_states) {
              ...
              real logit_v_other;
              if (v_true == 1) {
                logit_v_other = beta_v_true1;
              } else {
                logit_v_other = beta_v_true0;
              }
              logit_v_other += beta_dialects[dialect_id[other_latest_idx]];

              real logit_c_other;
              if (c_true == 1) {
                logit_c_other = beta_c_true1;
              } else {
                logit_c_other = beta_c_true0;
              }
      ...
      for (i in 1:N_states) {
        ...
        for (j in 1:N_states) {
          if (i != j) {
            ...
            // The key parameter to test Paul's hypothesis; only kicks in for regularizing transitions
            real irregularity_effect = gamma_irreg * irregularity_index[curr_lemma, time_point_idx] * is_regularizing;
      ...
      for (s in 1:N_states) {
        ...
        // SIMPLIFIED: logit calculation without splines
        real logit_v;
        if (v_true == 1) {
          logit_v = beta_v_true1;
        } else {
...
generated quantities {
  ...
  real paradigm_effect = gamma_irreg;  // Direct test of Paul's hypothesis
}

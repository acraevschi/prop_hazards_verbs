...
parameters {
  ...
  // Emission parameters (intercepts and frequency effects)
  real alpha_v_true1;                  // Vowel true=1 intercept
  real beta_v_true1_freq;              // Vowel true=1 frequency effect
  real alpha_v_true0;                  // Vowel true=0 intercept
  real beta_v_true0_freq;              // Vowel true=0 frequency effect
  real alpha_c_true1;                  // Consonant true=1 intercept
  real beta_c_true1_freq;              // Consonant true=1 frequency effect
  real alpha_c_true0;                  // Consonant true=0 intercept
  real beta_c_true0_freq;              // Consonant true=0 frequency effect
}
...
model {
  ...
  alpha_v_true1 ~ normal(0, 1);
  beta_v_true1_freq ~ normal(0, 1);
  alpha_v_true0 ~ normal(0, 1);
  beta_v_true0_freq ~ normal(0, 1);
  alpha_c_true1 ~ normal(0, 1);
  beta_c_true1_freq ~ normal(0, 1);
  alpha_c_true0 ~ normal(0, 1);
  beta_c_true0_freq ~ normal(0, 1);
  ...
  for (v in 1:N_verbs) {
    ...
    for (s in 1:4) {
      ...
      int curr_idx = obs_v_indices[1];

      // Compute emission logits using frequency
      real logit_v;
      if (v_true == 1) {
          logit_v = (alpha_v_true1 + beta_v_true1_freq * freq[curr_idx]);
      } else {
          logit_v = (alpha_v_true0 + beta_v_true0_freq * freq[curr_idx]);
      }
      
      real logit_c;
      if (c_true == 1) {
          logit_c = (alpha_c_true1 + beta_c_true1_freq * freq[curr_idx]);
      } else {
          logit_c = (alpha_c_true0 + beta_c_true0_freq * freq[curr_idx]);
      }

      log_forward[1][s] += bernoulli_logit_lpmf(obs_v[curr_idx] | logit_v) + bernoulli_logit_lpmf(obs_c[curr_idx] | logit_c);
    }
    ...
    for (t in 2:T) {
      ...
      for (i in 1:4) {
        ...
        for (j in 1:4) {
            if (i != j) {
                Q[i, j] = exp(log_lambda[i, j] + beta_trans * freq[prev_idx]) + 1e-9;
                total_rate += Q[i, j];
            }
        }
            Q[i, i] = -total_rate;

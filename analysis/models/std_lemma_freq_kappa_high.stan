functions {
  ...
  matrix get_transition_matrix(vector rates, real beta_lemma,
                               real beta_bipartite,
                               real lemma_eff,
                               real lemma_freq,
...
data {
  ...
  real lemma_freq[M, max_T];  // centre-transformed already
...
model {
  ...
  for (m in 1:M) {
    ...
    for (t in 1:(T_len[m] - 1)) {
      // lemma_freq, prop_bipartite at time t are used to form Q
      matrix[4,4] P = get_transition_matrix(
                        baseline_rates, beta_lemma, beta_bipartite,

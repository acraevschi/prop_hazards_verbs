...
df_lemma_uniq = df_merged.drop_duplicates(subset=["lemma_id", "std_infl"])

# Count lemma_ids attested before 1150 that have 'Pres' and either 'PastPl' or 'PastSg'
filtered = df_lemma_uniq[df_lemma_uniq["date"] <= 1200]
std_sets = filtered.groupby("lemma_id")["std_infl"].apply(set)
mask = std_sets.apply(lambda s: ("Pres" in s) and ("PastPl" in s) and ("PastSg" in s))
count = int(mask.sum())

valid_lemmas = std_sets.loc[mask].index.tolist()

df_merged[df_merged["lemma_id"].isin(valid_lemmas)]["lemma"].unique()
